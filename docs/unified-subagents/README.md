...
- **Agent mentions** — address live, resumable, or startable agents with `@handle message`; autocomplete merges agent handles with pi's file suggestions
...
```bash
pi install C:/Users/rahul/dev/pi_extension/pi-unified-subagents
```
...
The `general-purpose` agent is a **parent twin** — it receives the parent's entire system prompt plus a sub-agent context bridge, so it follows the same rules the parent does. Explore and Plan use standalone prompts tailored to their read-only roles. Explore has no pinned model, so caller-supplied `model` and `thinking` values work while its read-only tool restrictions remain intact.

Light is the fast, low-cost model for straightforward low-intelligence work. Task nature selects the agent: read-only search uses `Explore`, while implementation uses `general-purpose`. Thinking defaults to `low` and remains selectable; use a stronger model for difficult, open-ended, or high-risk work.
...
Frontmatter is authoritative. If an agent file sets `harness`, `model`, `thinking`, `max_turns`, `inherit_context`, `run_in_background`, `isolated`, or `isolation`, those values are locked for that agent. `Agent` tool parameters only fill fields the agent config leaves unspecified.
...
- Extensions bound into a subagent see **both ends** of that session's lifecycle: `session_start` when the agent starts and `session_shutdown` (reason `quit`) on eviction or quit. Handlers receive up to three seconds on quit.
- An installed **package** extension matches by its local package short name (`pi-unified-subagents` → `[pi-unified-subagents]`) in addition to its path-derived name. Prefer the package name.
...
`harness: "claude"` uses `@anthropic-ai/claude-agent-sdk` and the local Claude Code installation, authentication, settings, and native tools. It is available only in a trusted working directory because Claude runs headlessly with `bypassPermissions`; the extension refuses the spawn before the SDK starts when the project is untrusted. Claude's nested `Agent` and `Task` tools are always disabled to prevent recursive orchestration.

Claude v1 supports foreground and background runs, the shared queue and UI, model aliases/native IDs (optionally prefixed with `anthropic/`), thinking levels, max turns, tool restrictions, streaming, steering, stopping, and normal result delivery. Its current boundaries are explicit:

- `schedule`, `resume`, `inherit_context`, `isolated`, and `isolation: "worktree"` are rejected.
- Pi extension tools, MCP tools, skills, persistent memory, and Pi session persistence are not translated into Claude Code. `tools:` maps only the seven built-ins to Claude's native tools; `disallowed_tools:` narrows those tools further.
- Claude does not inherit the parent Pi model. With no `model`, Claude Code chooses its configured default.

Custom agents can pin the backend:

```yaml
---
description: Claude security review
harness: claude
tools: read, grep, find, bash
model: sonnet
thinking: high
...
`harness: "codex"` launches the separate [`@agentclientprotocol/codex-acp`](https://github.com/agentclientprotocol/codex-acp) stdio translation adapter, which translates ACP v1 requests to the bundled Codex `app-server`; OpenAI's Codex CLI does not currently expose native ACP mode. It uses existing Codex/ChatGPT or API-key authentication and runs only in trusted working directories. The client uses Codex's `agent` workspace mode and approves requested operations once for the active trusted run.

Codex supports foreground/background execution, the shared queue and UI, ACP text/thinking/tool/usage streaming, native model IDs (optionally prefixed with `openai/`), reasoning levels `low` through `max`, worktree isolation, ACP cancellation, and the adapter's `_session/steering` extension. Current boundaries are explicit:

- `schedule`, `resume`, `inherit_context`, `isolated`, and `max_turns` are rejected.
- Codex uses its native tools rather than Pi extension/MCP tools, skills, memory, or session persistence. Explicit extension/MCP or skill selections, memory, Pi session persistence, partial Pi `tools:` allowlists, `disallowed_tools:`, and `allowed_subagents:` are rejected; omit `tools:` or select the full built-in set. Omitted capability fields use native defaults and are distinct from explicit Pi selections.
- `prompt_mode: append` carries the parent system prompt plus the custom-agent body into the first ACP prompt; `replace` sends only the custom body. Worktree runs also receive an explicit parent-checkout exclusion block.
- Codex does not inherit the parent Pi model. With no `model`, Codex uses its configured default. Model selection is applied before reasoning validation because the adapter's available reasoning levels can change by model.
- Steering is accepted only when the adapter injects it into the active turn. A failed steer is reported; a race that would start a separate late turn is cancelled instead of being misreported as active-turn steering.
- On completion or cancellation, the client closes the ACP session before tearing down the stdio adapter, with a bounded kill fallback.
...
Settings                                    ← runtime settings + global Light model
```
...
- **Settings** — configure runtime behavior plus the global Light model
...
|-------|------|------------|
| `subagents:created` | Background agent registered | `id`, `type`, `harness`, `description`, `isBackground` |
| `subagents:started` | Agent transitions to running (including queued→running) | `id`, `type`, `harness`, `description` |
| `subagents:completed` | Agent finished successfully (background and foreground) | `id`, `type`, `harness`, `durationMs`, display `tokens`, billed `usage` (including `cacheRead` and `cost.total`, absent when nothing was spent), `toolUses`, `result` |
...
| `subagents:ready` | RPC handlers registered and armed — fired on session start; not emitted in a session that excludes this extension | — |
...
`subagents:ready` fires only when this extension is actually loaded **and bound** in the current session. A session that excludes it (via an agent's `extensions:`) emits no `subagents:ready` and does not answer the RPC channels — exactly as if the extension were not installed. Treat "no `subagents:ready`" as "not available here" and give discovery a timeout rather than waiting indefinitely.
...
src/
  ...
  agent-file-toggle.ts # Locate/edit an agent's .md and serialize frontmatter
  agent-color.ts       # Agent name color parsing and badge rendering

  # Execution
  backend.ts           # Shared session/backend contract
  ...
  child-context.ts     # AsyncLocalStorage child-session marker
  abortable.ts         # Detachable foreground wait helper
  cross-extension-rpc.ts # Cross-extension spawn/ping/consume RPC
...
MIT. Upstream copyright and attribution are preserved in [`LICENSE`](LICENSE).
