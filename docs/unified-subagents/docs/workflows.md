...
Adapted from upstream v0.19 for Pi Toolkit. Workflow children use Toolkit's existing agent-file harness/model policy; Pi, Claude, Codex, and Agy remain available through their definitions or `agent(..., { harness })`; definition pins retain precedence. `schema` and child-session `resume` require the Pi harness; unsupported combinations fail explicitly. Workflows share the existing Tasks/Agents Activity surface, never a second legacy widget.
...
A **`workflow` row in the existing Agents Activity tab**, above ordinary agent rows, carrying its agent counts where a description would go. `⏎` on it opens the inspector rather than a conversation overlay.
...
A run's own agents are not listed separately in the Activity list, the `/agents` menus or `@handle` resolution — they belong to the run, which reports for them. `c` in the inspector is the one way in to a child's conversation.
...
Only the second stage changed, so re-running with `resumeFromRunId` replays the unchanged leading calls up to the first changed verify call. That call and everything after it run live; pipelined audit/verify calls may interleave, so not every audit result is necessarily replayed. The notification says so: `… , 7 replayed from wf_9f3ab21c04de`.
...
At least one of `script` / `scriptPath` / `name` is required unless `resumeFromRunId` supplies the prior script; `scriptPath` wins over `script`, which wins over `name`.
...
| `harness` | string | Toolkit addition: `pi`, `claude`, `codex`, or `agy`; agent-file pins retain precedence |
| `model` | string | Pi: `provider/modelId` or fuzzy like `haiku`; native harnesses: their own model IDs |
...
Combination rules: `resume` cannot be combined with `agentType`, `harness`, `model`, `effort`, `isolation`, `gate` or `schema` — a resumed child keeps the agent type, model and tree it was started with, and its session predates the `StructuredOutput` tool.
...
`workflowsEnabled` is **on**; leaving it unset means *auto*, which is on unless another extension already offers a `Workflow`, `workflow`, or `SubagentWorkflow` tool, in which case this one stands down for the session. Setting it explicitly pins it. See [Persistent settings](../README.md#persistent-settings).
...
A typo, or an option from a different tool. The supported set is `label`, `phase`, `harness`, `model`, `agentType`, `isolation`, `gate`, `resume`, `effort`, `schema`.
...
Every file below is executed by `test/unified-subagents/workflow-examples.test.ts` against a stub host in the local regression suite.
