...
export function validateSettings(raw: unknown): SettingsConfig {
  ...
  const footerEnabled = typeof r.footerEnabled === "boolean" ? r.footerEnabled : DEFAULT_SETTINGS.footerEnabled;
  const showFullPath = typeof r.showFullPath === "boolean" ? r.showFullPath : DEFAULT_SETTINGS.showFullPath;
  const segments = validateSegments(r.segments);
  const contextZones = validateZones(r.contextZones);

  return { version: 1, preset, footerEnabled, showFullPath, segments, contextZones };
}
