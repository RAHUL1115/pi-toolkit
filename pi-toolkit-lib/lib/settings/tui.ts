...
export function toSettingsListItems(config: SettingsConfig): SettingsListItem[] {
  const items: SettingsListItem[] = [
    {
      id: "footerEnabled",
      label: "Pi Toolkit Footer",
      description: "Use the Pi Toolkit footer instead of Pi's default footer",
      currentValue: config.footerEnabled ? "true" : "false",
      values: ["true", "false"],
    },
    {
      id: "showFullPath",
      label: "Full Working Path",
      description: "Show the full working path instead of only the folder name",
