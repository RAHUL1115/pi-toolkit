...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerCommand("ptk-footer-settings", {
    description: "Configure the footer, layout, segments, and context zones",
    handler: async (_args, ctx) => {
      if (!ctx.hasUI) {
        ...
        function rebuildSettingsList() {
          settingsList = new SettingsList(
            ...
            async (id, newValue) => {
              ...
              await saveSettings(config, settingsStorage);
              if (id === "footerEnabled") {
                if (config.footerEnabled) setupFooter(ctx);
                else teardownFooter(ctx);
              }
              tui.requestRender();
            },
            done,
