 ...
 * including its BOM normalization. The BOM stays in `lines[0]`, preserving the
 * user's file encoding.
 *
...
function splitFrontmatter(content: string):
  ...
  const first = (content.startsWith("\uFEFF") ? lines[0].slice(1) : lines[0]).replace(/\r?\n$/, "");
...
export interface NewAgentInput {
  description: string;
  /** Native execution harness; omitted or pi keeps the default. */
...
export function buildNewAgentFile(input: NewAgentInput): string {
  const harnessLine = input.harness && input.harness !== "pi" ? `\nharness: ${input.harness}` : "";
  const modelLine = input.model ? `\nmodel: ${JSON.stringify(input.model)}` : "";
  const thinkingLine = input.thinking ? `\nthinking: ${input.thinking}` : "";
  return `---
description: ${JSON.stringify(input.description)}
tools: ${input.tools}${harnessLine}${modelLine}${thinkingLine}
prompt_mode: replace
---
...
export function serializeAgentFile(cfg: AgentConfig): string {
  ...
  if (cfg.harness && cfg.harness !== "pi") fmFields.push(`harness: ${cfg.harness}`);
  ...
  if (cfg.maxTurns !== undefined) fmFields.push(`max_turns: ${cfg.maxTurns}`);
  if (cfg.persistSession) fmFields.push("persist_session: true");
  if (cfg.sessionDir) fmFields.push(`session_dir: ${cfg.sessionDir}`);
  ...
  if (cfg.outputTranscript !== undefined) fmFields.push(`output_transcript: ${cfg.outputTranscript}`);
