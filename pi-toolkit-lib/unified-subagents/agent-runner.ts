...
export type { ToolActivity } from "./backend.js";

/** Pi-only bridge carried through the unified backend seam. */
export interface NestedRuntimeOptions {
  // The unified manager exposes backend-neutral sessions; nested tools are
  // injected only into Pi sessions, where that session is an AgentSession.
  manager: unknown;
...
// The manager dispatches through BackendRunOptions before the Pi adapter reaches
// this runner. Keep nesting in that shared envelope; non-Pi adapters ignore it.
declare module "./backend.js" {
  interface BackendRunOptions {
    nestedRuntime?: NestedRuntimeOptions;
  }
}

/** Native runner options retain the full Pi session in its public callback. */
export type RunOptions = Omit<BackendRunOptions, "onSessionCreated"> & {
  onSessionCreated?: (session: AgentSession) => void;
};

/** Native runner results retain the full Pi session in their public shape. */
export type RunResult = Omit<BackendRunResult, "session"> & {
  session: AgentSession;
};

export type ResumeOptions = BackendResumeOptions;
export type ResumeResult = BackendResumeResult;
...
export async function runAgent(
  ...
  const nestedTools = agentConfig?.allowedSubagents && nestedRuntime && !options.isolated
    ? createNestedSubagentTools({
        manager: nestedRuntime.manager as NestedAgentManager,
        pi: options.pi,
 ...
 */
export function getAgentConversation(session: SubagentSession): string {
  const parts: string[] = [];
