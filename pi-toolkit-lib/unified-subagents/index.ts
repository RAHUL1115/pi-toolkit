 ...
 *   SubagentWorkflow    — LLM-callable: orchestrate scripted sub-agent runs
...
import { AgentManager, DEFAULT_AUTO_BACKGROUND_MS, isTopLevelAgent } from "./agent-manager.js";
...
import {
  ...
  getLightModel,
  getLightThinking,
  type LightThinkingLevel,
  loadSettings,
  persistToastFor,
  reconcileLightModel,
  type SubagentsSettings,
  saveAndEmitChanged,
  saveGlobalLightSettings,
  setLightModel,
  setLightThinking,
...
export function renderRunningAgentStatus(
  ...
  container.addChild(new Text(theme.fg("dim", "  ctrl+b to run in background"), 0, 0));
  return container;
}

/** Full model IDs for light-model settings, retaining a configured model that is currently unavailable. */
export function getLightModelChoices(registry: ModelRegistry, current: string): string[] {
  const available = (registry.getAvailable?.() ?? [])
    .filter((model): model is { provider: string; id: string } =>
      typeof model?.provider === "string" && typeof model?.id === "string")
    .map(model => `${model.provider}/${model.id}`);
  return [...new Set([...available, current])].sort((a, b) => a.localeCompare(b));
}
...
function createActivityTracker(maxTurns?: number, onStreamUpdate?: () => void) {
  ...
  const callbacks = {
    ...
    onSessionCreated: (session: SubagentSession) => {
...
function buildDetails(
  ...
  record: { toolUses: number; startedAt: number; completedAt?: number; status: string; error?: string; id?: string; session?: SubagentSession; lifetimeUsage: LifetimeUsage },
...
/** Public workflow entry identifiers used by transcript consumers and tests. */
export { FOREIGN_WORKFLOW_TOOL_NAMES, WORKFLOW_ENTRY_TYPE, type WorkflowEntryData, workflowEntryData };

/** Persist only an explicit workflow setting; automatic collision stand-down is session-local. */
export function workflowSettingSnapshot(pinned: boolean, enabled: boolean): boolean | undefined {
...
export function registerUnifiedSubagents(pi: ExtensionAPI, tasks?: BackgroundTaskController): void {
  ...
  const manager = new AgentManager((record) => {
    ...
    pi.appendEntry("subagents:record", {
      id: record.id, type: record.type, harness: record.harness, description: record.description,
  ...
  /** Resolve a programmatic spawn through the same harness policy as Agent. */
  const spawnResolved = (piRef: any, ctxRef: ExtensionContext, type: string, prompt: string, options: any) => {
    reloadCustomAgents();
    const dispatch = resolveSpawnType(type);
    if (!dispatch.ok) throw new Error(dispatch.message);
    const config = getAgentConfig(dispatch.type);
    const resolved = resolveHarnessInvocation({
      ctx: ctxRef,
      config,
      params: options ?? {},
    ...
    const id = manager.spawn(piRef, ctxRef, dispatch.type, prompt, {
      ...options,
      ...callbacks,
      description: typeof options?.description === "string" ? options.description : dispatch.type,
      ...
      cwd: resolved.cwd as string | undefined,
      invocation: resolved.invocation,
    });
    agentActivity.set(id, state);
    if (ctxRef.hasUI) {
      fleet.setUICtx(ctxRef.ui as unknown as FleetUICtx);
      fleet.ensureTimer();
      fleet.update();
    }
    return id;
  ...
  const spawnTopLevel = (piRef: any, ctxRef: ExtensionContext, type: string, prompt: string, options: any) => {
    const safeOptions = { ...(options ?? {}) };
    for (const key of [
      "parentAgentId", "workflowId", "depth", "maxSubagentDepth", "configCwd", "rootSessionId",
      "resumeSessionFile", "reclaim", "trusted", "agentConfig", "modelHint",
      "invocation", "blocking", "bypassQueue", "signal", "onToolActivity", "onTextDelta",
      "onSessionCreated", "onTurnEnd", "onAssistantUsage", "onCompaction",
  ...
  let lightModelInitialized = false;
  ...
  pi.on("session_start", async (_event, ctx) => {
    // The registrar survives /new and /resume; only the replaced session was fenced.
    ...
    const reconciledLightModel = reconcileLightModel(
      ctx.model?.provider,
      ctx.modelRegistry.getAvailable(),
      getLightModel(),
      lightModelInitialized,
    );
    if (reconciledLightModel) {
      setLightModel(reconciledLightModel.lightModel);
      const settings = { ...reconciledLightModel, lightThinking: getLightThinking() };
      const persisted = saveGlobalLightSettings(settings.lightModel, settings.lightThinking);
      lightModelInitialized = persisted;
      pi.events.emit("subagents:settings_changed", { settings, persisted });
      const reason = persisted ? "Light model initialized" : "Light model updated for this session";
      ctx.ui.notify(`${reason}. Use /reload to refresh Agent guidance.`, persisted ? "info" : "warning");
    }
  ...
  pi.on("session_shutdown", async () => {
    // Successful /new, /resume and /fork flows replace the bound session while
    // this registrar survives. Fence detached workflow continuations before
    // aborting so they cannot append entries or notify the replacement session.
    ...
    workflowTasks.clear();
    manager.abortAll();
    // runWorkflow resolves only after its worker thread terminates. Give those
    // resources a bounded chance to settle; manager.dispose below remains the
    // separate bounded cleanup for child agent sessions.
    await settleWorkflowRuns();
  ...
  function getEffectiveWidgetMode(): WidgetMode { return "off"; }
  ...
  lightModelInitialized = loadedSettings.lightModel !== undefined;
...
**Never delegate understanding.** Don't write "based on your findings, fix the bug" or "based on the research, implement it." Those phrases push synthesis onto the agent instead of doing it yourself. Write prompts that prove you understood: include file paths, line numbers, what specifically to change.`;
  ...
  const agentTool = defineTool({
    ...
    parameters: Type.Object({
      ...
      harness: Type.Optional(
        Type.Union(AGENT_HARNESSES.map(value => Type.Literal(value)), {
          description: 'Execution harness. Defaults to "pi". Custom-agent frontmatter overrides this parameter.',
        }),
      ),
      model: Type.Optional(
        Type.String({
    ...
    execute: async (toolCallId, params, signal, onUpdate, ctx) => {
      ...
      try {
        resolvedHarness = resolveHarnessInvocation({
          ...
          config: customConfig,
          params,
          operation: params.resume ? "resume" : params.schedule ? "schedule" : "spawn",
      ...
      for (const warning of resolvedHarness.warnings) ctx.ui.notify(warning, "warning");
      ...
      const agentInvocation: AgentInvocation = {
        ...resolvedHarness.invocation,
        // Explicit value only — the default fallback would just add noise.
        // Normalize so `0` (unlimited) doesn't surface as a misleading "max turns: 0".
        maxTurns: normalizeMaxTurns(resolvedHarness.invocation.maxTurns),
      };
      ...
      if (runInBackground) {
        ...
        bgCallbacks.onSessionCreated = (session: SubagentSession) => {
      ...
      fgCallbacks.onSessionCreated = (session: SubagentSession) => {
      ...
      try {
        const fgResult = await manager.spawnAndWait(pi, ctx, subagentType, params.prompt, {
          ...
          // Set up background-delivery metadata and the output transcript before
          // Ctrl+B or onSessionCreated can observe the record.
          fgId = fgAgentId;
          const fgRec = manager.getRecord(fgAgentId);
          if (fgRec) {
            fgRec.joinMode = "async";
      ...
      if (record.isBackground) {
        if (record.status === "running") agentActivity.set(record.id, fgState);
        widget.ensureTimer();
        widget.update();
        fleet.ensureTimer();
        fleet.update();
        return textResult(
          ...
          `Use get_subagent_result to retrieve full results, or steer_subagent to send it messages.\n` +
          `Do not duplicate this agent's work.`,
          buildDetails(detailBaseFor(record), record, fgState, { tokens: tokenText, status: "background" }),
        );
      }
  ...
  const WORKFLOW_SHUTDOWN_WAIT_MS = 2_000;

  /** Track a detached run, fence its completion side effects, and bound shutdown. */
  function launchWorkflowTask(ctx: ExtensionContext, task: WorkflowTask, onSettled: () => void): void {
    const run = runWorkflowTask(ctx, task)
      .then(() => {
        if (workflowSessionActive && workflowTasks.get(task.id) === task) onSettled();
      })
      .finally(() => { workflowRuns.delete(run); });
    workflowRuns.add(run);
  }

  async function settleWorkflowRuns(): Promise<void> {
    ...
    try {
      await Promise.race([
        ...
        new Promise<void>(resolve => {
          timer = setTimeout(resolve, WORKFLOW_SHUTDOWN_WAIT_MS);
          timer.unref?.();
        }),
      ]);
    } finally {
  ...
  const workflowTool = defineTool({
    ...
    execute: async (toolCallId, params, _signal, _onUpdate, ctx) => {
      ...
      launchWorkflowTask(ctx, task, () => notifyWorkflowFinished(task));
  ...
  registerToolReportingUsage(defineTool({
    ...
    renderResult(result, { expanded }, theme) {
      ...
      const linePreview = lines.slice(0, 10).join("\n");
      const preview = linePreview.slice(0, 1200).trimEnd();
      const truncated = lines.length > 10 || linePreview.length > 1200;
      if (!truncated) return new Text(text, 0, 0);
  ...
  registerToolReportingUsage(defineTool({
    ...
    execute: async (_toolCallId, params, _signal, _onUpdate, _ctx) => {
      ...
      if (!record.session) {
        ...
        pi.events.emit("subagents:steered", { id: record.id, harness: record.harness, message: params.message });
...
Write the file using the write tool. Only write the file, nothing else.`;
  ...
  async function showManualWizard(ctx: ExtensionCommandContext, targetDir: string) {
    ...
    // 3. Harness
    const harnessChoice = await ctx.ui.select("Harness", [...AGENT_HARNESSES]);
    if (!harnessChoice) return;
    // 4. Tools
    let tools = BUILTIN_TOOL_NAMES.join(", ");
    if (harnessChoice !== "codex") {
    ...
    const content = buildNewAgentFile({
      description,
      harness: harnessChoice as AgentConfig["harness"],
      tools,
      model,
  ...
  function snapshotSettings() {
    return {
      ...
      lightThinking: getLightThinking(),
      ...
      workflowsEnabled: workflowSettingSnapshot(isWorkflowsPinned(), isWorkflowsEnabled()),
  ...
  async function showSettings(ctx: ExtensionCommandContext) {
    function buildItems(): SettingItem[] {
      ...
      const lightThinking = getLightThinking();
      ...
      return [
        ...
        },
        {
          id: "lightModel",
          label: "Light model",
          description: "Fast, low-cost model for straightforward low-intelligence tasks",
          currentValue: lightModel,
          values: getLightModelChoices(ctx.modelRegistry, lightModel),
        },
        {
          id: "lightThinking",
          label: "Light thinking",
          description: "Thinking level used with the Light model",
          currentValue: lightThinking,
        ...
        },
        {
          ...
          description: "Markdown rendering in conversation viewers: assistant, all, or off. Press m in a viewer to cycle.",
    ...
    function applyValue(id: string, value: string) {
      if (id === "lightModel") {
        setLightModel(value);
        notifyGlobalModelApplied(ctx, `Light model set to ${value}. Use /reload to refresh Agent guidance.`);
      } else if (id === "lightThinking") {
        setLightThinking(value as LightThinkingLevel);
        notifyGlobalModelApplied(ctx, `Light thinking set to ${value}. Use /reload to refresh Agent guidance.`);
  ...
  function notifyGlobalModelApplied(ctx: ExtensionCommandContext, successMsg: string) {
    const settings = { lightModel: getLightModel(), lightThinking: getLightThinking() };
    const persisted = saveGlobalLightSettings(settings.lightModel, settings.lightThinking);
    pi.events.emit("subagents:settings_changed", { settings, persisted });
