...
export interface NestedAgentManager {
  ...
  /** Resolves when async startup completes; rejects on worktree creation failure. */
...
export function createNestedSubagentTools(context: NestedToolContext): ToolDefinition[] {
  ...
  const agentTool = defineTool({
    ...
    execute: async (_toolCallId, params, signal, _onUpdate, ctx) => {
      ...
      // Nested delegation stays foreground by default and obeys the project
      // worktree switch. Native harnesses remain unsupported for tracked nesting.
      ...
      if (invocation.harness !== "pi") {
        return textResult(
          `Nested delegation currently supports only the Pi harness; "${resolvedType}" is pinned to ${invocation.harness}.`,
          true,
        );
      }
      let model = ctx.model;
