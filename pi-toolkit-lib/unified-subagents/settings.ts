...
// - Global:  ~/.pi/agent/subagents.json (via getAgentDir()) — the light model is written here
// - Project: <cwd>/.pi/subagents.json — other /agents settings; overrides global on load
...
export const DEFAULT_LIGHT_MODEL = "openai-codex/gpt-5.6-sol";
export const DEFAULT_LIGHT_THINKING: LightThinkingLevel = "low";

let lightModel = DEFAULT_LIGHT_MODEL;
let lightThinking: LightThinkingLevel = DEFAULT_LIGHT_THINKING;

export function getLightModel(): string { return lightModel; }
export function setLightModel(model?: string): void {
  lightModel = model?.trim() || DEFAULT_LIGHT_MODEL;
}
export function getLightThinking(): LightThinkingLevel { return lightThinking; }
export function setLightThinking(thinking?: LightThinkingLevel): void {
  lightThinking = thinking ?? DEFAULT_LIGHT_THINKING;
}
...
export interface AvailableLightModel {
  provider: string;
  id: string;
}
...
export interface SubagentsSettings {
  ...
  /** Global-only light configuration. Project values are ignored. */
  lightModel?: string;
  lightThinking?: LightThinkingLevel;
  /**
...
function sanitize(raw: unknown, includeLightModel = false): SubagentsSettings {
  ...
  if (includeLightModel && typeof r.lightModel === "string" && r.lightModel.trim()) {
    out.lightModel = r.lightModel.trim();
  }
  if (includeLightModel && typeof r.lightThinking === "string" && VALID_THINKING_LEVELS.has(r.lightThinking)) {
    out.lightThinking = r.lightThinking as LightThinkingLevel;
  }
...
function fullModelName(model: AvailableLightModel): string {
  return `${model.provider}/${model.id}`;
}

/**
 * Initialize one light model for the active provider, then repair it when the
 * provider changes or the configured model disappears. Registry order is
 * authoritative; the last matching entry is the provider's latest model.
 */
export function reconcileLightModel(
  currentProvider: string | undefined,
  available: AvailableLightModel[],
  configured: string | undefined,
  initialized: boolean,
): LightModelSettings | undefined {
  if (!currentProvider) return undefined;
  const providerModels = available.filter(model => model.provider === currentProvider);
  const latest = providerModels.at(-1);
  if (!latest) return undefined;

  const configuredAvailableForProvider = configured !== undefined && providerModels
    .some(model => fullModelName(model) === configured);
  if (initialized && configuredAvailableForProvider) return undefined;

  const provider = currentProvider.toLowerCase();
  const preferred = provider === "openai" || provider === "openai-codex"
    ? providerModels.find(model => model.id === "gpt-5.6-sol")
    : provider === "anthropic"
      ? providerModels.filter(model => model.id.toLowerCase().includes("sonnet")).at(-1)
      : undefined;

  return { lightModel: fullModelName(preferred ?? latest) };
}
 ...
 */
function readSettingsFile(path: string, includeLightModel = false): SubagentsSettings {
  if (!existsSync(path)) return {};
  try {
    return sanitize(JSON.parse(readFileSync(path, "utf-8")), includeLightModel);
  } catch (err) {
...
/** Load merged settings: global provides defaults, project overrides non-model fields. */
export function loadSettings(cwd: string = process.cwd()): SubagentsSettings {
  return { ...readSettingsFile(globalPath(), true), ...readSettingsFile(projectPath(cwd)) };
}

/**
 * Write project-local settings. This function never touches the global file.
...
export function saveSettings(s: SubagentsSettings, cwd: string = process.cwd()): boolean {
  ...
  for (const key of LIGHT_MODEL_KEYS) delete projectSettings[key];
  try {
...
/** Merge the light configuration into valid global JSON and remove legacy dual-model fields. */
export function saveGlobalLightSettings(model: string, thinking: LightThinkingLevel): boolean {
  ...
  try {
    ...
    const settings = sanitize({ lightModel: model, lightThinking: thinking }, true);
    if (!settings.lightModel || !settings.lightThinking) return false;
    for (const key of LEGACY_LIGHT_MODEL_KEYS) delete existing[key];

    mkdirSync(dirname(path), { recursive: true });
    writeFileSync(path, JSON.stringify({
      ...existing,
      lightModel: settings.lightModel,
      lightThinking: settings.lightThinking,
    }, null, 2), "utf-8");
    return true;
  } catch {
    return false;
