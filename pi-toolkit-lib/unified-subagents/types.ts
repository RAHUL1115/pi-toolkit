...
export interface AgentConfig {
  ...
  /** Optional UI label; `name` identifies the agent type. */
  displayName?: string;
  /** Claude Code-compatible name color (named color or #RRGGBB). */
  color?: string;
  description: string;
  builtinToolNames?: string[];
  /** Whether `tools:` was explicitly present in custom-agent frontmatter. */
  ...
  /** Whether an extensions/inherit_extensions field was explicitly present. */
  ...
  /** Whether a skills/inherit_skills field was explicitly present. */
...
export interface AgentRecord {
  id: string;
  type: SubagentType;
  /** Execution harness selected for this run. */
  harness: AgentHarness;
  /** Typeable name for `@handle message`; top-level agents only. */
  handle?: string;
  /** Optional memorable alias from the spawn's `name`. */
  alias?: string;
  description: string;
  ...
  /**
   * Whether this agent currently runs in the background. Tri-state, initialized
   * from `SpawnOptions.isBackground` and changed to `true` by Ctrl+B detachment:
   * `true` = background, `false` = foreground (has an inline Agent tool-result surface), `undefined` = the
  ...
  /** Resolved spawn params for UI display; runInBackground updates on detachment. */
...
export interface AgentInvocation {
  /** Harness tag for backend-neutral surfaces. */
  harness?: AgentHarness;
  /** Short display name for tight rows, e.g. "haiku 4.5". */
  modelName?: string;
