...
import { resolveHarnessInvocation, type HarnessResolution } from "../harness-resolution.js";
import type { AgentRecord, AgentHarness } from "../types.js";
...
function harnessLabel(harness: AgentHarness): string {
  return harness === "pi" ? "Pi" : harness === "claude" ? "Claude" : harness === "codex" ? "Codex ACP" : "Agy";
}
...
export function createWorkflowHost(deps: WorkflowHostOptions): WorkflowHost {
  ...
  return {
    async spawnAgent(request) {
      ...
      // Use the same authoritative harness/model/policy resolution as every
      // other spawn path. This preserves local frontmatter precedence, model
      // scope, trust checks, and native harness capability validation.
      const config = getAgentConfig(dispatch.type);
      let resolved: HarnessResolution;
      try {
        resolved = resolveHarnessInvocation({
          ctx,
          config,
          params: {
      ...
      if (request.schema !== undefined && resolved.harness !== "pi") {
        return {
          ok: false,
          error: `${harnessLabel(resolved.harness)} workflow agents do not support schema/StructuredOutput. Use the Pi harness or omit schema.`,
        };
      }
      for (const warning of resolved.warnings) {
        if (warnedScopeMessages.has(warning)) continue;
        warnedScopeMessages.add(warning);
      ...
      try {
        const { record } = await manager.spawnAndWait(
          ...
          request.prompt,
          {
            ...
            ...(resolved.modelHint !== undefined ? { modelHint: resolved.modelHint } : {}),
            ...(resolved.trusted !== undefined ? { trusted: resolved.trusted } : {}),
            ...(resolved.maxTurns !== undefined ? { maxTurns: resolved.maxTurns } : {}),
            ...(resolved.thinking !== undefined ? { thinkingLevel: resolved.thinking } : {}),
            ...
            ...(resolved.isolation !== undefined ? { isolation: resolved.isolation } : {}),
            ...(deps.signal !== undefined ? { signal: deps.signal } : {}),
    ...
    async resumeAgent(agentId, prompt, onResolved) {
      ...
      if (existing !== undefined && existing.harness !== "pi") {
        return {
          ok: false,
          error: `${harnessLabel(existing.harness)} workflow agents do not support resume.`,
        };
      }
