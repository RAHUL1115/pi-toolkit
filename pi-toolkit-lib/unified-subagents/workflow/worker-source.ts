...
async function agentIn(scope, prompt, opts) {
  ...
  const harness = optionalText(options.harness, "agent() opts.harness");
  if (harness !== undefined && ["pi", "claude", "codex", "agy"].indexOf(harness) === -1) {
    throw new Error("agent() opts.harness must be one of: pi, claude, codex, agy.");
  }
  ...
  if (resume !== undefined) {
    ...
    if (harness !== undefined) {
      throw new Error(
        "agent() opts.resume and opts.harness are mutually exclusive: a resumed agent keeps the harness it was started with."
      );
    }
