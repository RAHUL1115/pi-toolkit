 ...
 *
 * Git runs through the async ExtensionAPI so worktree copies do not block the TUI.
 */
...
/** Run git and reproduce execFileSync's throw-on-failure behavior. */
...
async function removeWorktree(pi: ExtensionAPI, cwd: string, worktreePath: string): Promise<void> {
  try {
    ...
    try { await git(pi, cwd, ["worktree", "prune"], 5000); } catch { /* ignore */ }
  }
}
