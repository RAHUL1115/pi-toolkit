...
import { resumeAgent, runAgent } from "../../pi-toolkit-lib/unified-subagents/agent-runner.js";
import { addUsage } from "../../pi-toolkit-lib/unified-subagents/usage.js";
import { isWorktreeIsolationEnabled } from "../../pi-toolkit-lib/unified-subagents/worktree.js";
...
describe("AgentManager — lifetime usage + compaction count are eagerly initialized", () => {
  ...
  it("resume() also accumulates usage and increments compactions on the same record", async () => {
    ...
    const { resumeAgent: resumeMock } = await import("../../pi-toolkit-lib/unified-subagents/agent-runner.js");
...
describe("AgentManager — isolation: worktree fails loud, no silent fallback", () => {
  ...
  it("awaitStartup rejects when createWorktree returns undefined; no orphan record remains", async () => {
    const { createWorktree } = await import("../../pi-toolkit-lib/unified-subagents/worktree.js");
...
describe("AgentManager — SpawnOptions.cwd passthrough (#96)", () => {
  ...
  it("cwd + isolation: worktree — worktree created FROM cwd, session runs at the copy's workPath, cleanup targets cwd's repo", async () => {
    const { createWorktree, cleanupWorktree } = await import("../../pi-toolkit-lib/unified-subagents/worktree.js");
    vi.mocked(createWorktree).mockClear();
