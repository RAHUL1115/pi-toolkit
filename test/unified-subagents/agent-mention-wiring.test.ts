...
vi.mock("../../pi-toolkit-lib/unified-subagents/mention-clone.js", () => ({ runMentionClone: vi.fn() }));

import { getDefaultMaxTurns, resumeAgent, runAgent, setDefaultMaxTurns } from "../../pi-toolkit-lib/unified-subagents/agent-runner.js";
...
describe("mentioning an agent that has never run", () => {
  ...
  it("shows the turn limit it will actually be held to (#181)", async () => {
    ...
    try {
      ...
      await lifecycle.get("tool_execution_start")?.({}, uiCtx);
