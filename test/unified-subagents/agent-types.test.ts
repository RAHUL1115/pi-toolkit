...
describe("agent type registry", () => {
  ...
  describe("default agents", () => {
    ...
    it("does not treat the light model label as an agent type", () => {
      expect(isValidType("Light")).toBe(false);
    ...
    it("Explore inherits the caller model while keeping its read-only role", () => {
