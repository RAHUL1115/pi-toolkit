...
import { renderRunningAgentStatus } from "../../pi-toolkit-lib/unified-subagents/index.js";
import type { WidgetMode } from "../../pi-toolkit-lib/unified-subagents/types.js";
import { type AgentActivity, AgentWidget, buildInvocationTags, fgPreservingNestedStyles, formatCost, formatSessionTokens, formatTurns } from "../../pi-toolkit-lib/unified-subagents/ui/agent-widget.js";

describe("formatTurns", () => {
  it("separates the turn glyph from its count to prevent visual overlap", () => {
    expect(formatTurns(9)).toBe("↻ 9");
    expect(formatTurns(9, 30)).toBe("↻ 9≤30");
  });
});
...
describe("buildInvocationTags", () => {
  it("keeps Pi implicit and labels native harnesses", () => {
    expect(buildInvocationTags({ harness: "pi" }).tags).toEqual([]);
    expect(buildInvocationTags({ harness: "claude" }).tags).toEqual(["claude"]);
    expect(buildInvocationTags({ harness: "codex" }).tags).toEqual(["codex"]);
  });
});
