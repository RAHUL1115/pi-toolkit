...
import { serializeAgentFile } from "../../pi-toolkit-lib/unified-subagents/agent-file-toggle.js";
import { BUILTIN_TOOL_NAMES } from "../../pi-toolkit-lib/unified-subagents/agent-types.js";
...
Just a prompt.`);
    ...
    expect(agent.harness).toBeUndefined();
  ...
  it("preserves harness when serializing an ejected agent", () => {
    ...
    writeAgent("native", content);

    expect(content).toContain("\nharness: claude\n");
    expect(loadCustomAgents(tmpDir).get("native")).toMatchObject({
      harness: "claude",
      systemPrompt: "Use Claude Code.",
    });
  });

  it("preserves Codex harness when serializing an ejected agent", () => {
    const content = serializeAgentFile({
      name: "codex-native",
      harness: "codex",
      ...
      extensions: false,
      skills: false,
      systemPrompt: "Use Codex through ACP.",
      promptMode: "replace",
    });
    writeAgent("codex-native", content);

    expect(content).toContain("\nharness: codex\n");
    expect(loadCustomAgents(tmpDir).get("codex-native")).toMatchObject({
      harness: "codex",
      extensions: false,
      skills: false,
    });

    writeAgent("codex-defaults", `---
harness: codex
---

Use native Codex capabilities.`);
    expect(loadCustomAgents(tmpDir).get("codex-defaults")).toMatchObject({
      harness: "codex",
      extensions: false,
      skills: false,
    });
  ...
  it("distinguishes omitted native capability fields from explicit selections", () => {
    writeAgent("implicit", `---
description: Implicit native defaults
---

Use native defaults.`);
    writeAgent("explicit", `---
description: Explicit Pi capabilities
tools: all
extensions: true
skills: true
---
...
Use explicit capabilities.`);
    ...
    const result = loadCustomAgents(tmpDir);
    expect(result.get("implicit")).toMatchObject({
      toolsExplicit: false,
      extensionsExplicit: false,
      skillsExplicit: false,
    });
    expect(result.get("explicit")).toMatchObject({
      toolsExplicit: true,
  ...
  it("serializes all persisted agent behavior fields through one formatter", () => {
    const content = serializeAgentFile({
      name: "complete",
      harness: "codex",
      ...
      allowedSubagents: ["scout", "reviewer"],
    ...
    expect(content).toContain("max_turns: 0");
    expect(content).toContain("persist_session: true");
    expect(content).toContain("session_dir: .pi/sessions");
    expect(content).toContain("allowed_subagents: scout, reviewer");
    expect(content).toContain("output_transcript: true");
    expect(content).toContain("enabled: false");
  });

  it("warns and defaults invalid harness values to Pi", () => {
    const warn = vi.spyOn(console, "warn").mockImplementation(() => {});
    writeAgent("bad-harness", `---
harness: unknown
---

Invalid harness.`);

    const agent = loadCustomAgents(tmpDir).get("bad-harness")!;
    expect(agent.harness).toBeUndefined();
    expect(warn).toHaveBeenCalledWith(expect.stringContaining("invalid harness"));
  });
