...
describe("documented defaults (README:441)", () => {
  ...
  it("grace turns after the soft limit default to 5", async () => {
    const { getGraceTurns } = await import("../../pi-toolkit-lib/unified-subagents/agent-runner.js");
    expect(getGraceTurns()).toBe(5);
  }, HEAVY_REIMPORT_MS);

  it("max turns is unlimited by default", async () => {
    const { getDefaultMaxTurns } = await import("../../pi-toolkit-lib/unified-subagents/agent-runner.js");
    expect(getDefaultMaxTurns()).toBeUndefined();
  }, HEAVY_REIMPORT_MS);

  it("nested subagent depth defaults to 2", async () => {
    const { getMaxSubagentDepth } = await import("../../pi-toolkit-lib/unified-subagents/nested-tools.js");
  ...
  it("background concurrency defaults to 10", async () => {
    const { AgentManager } = await import("../../pi-toolkit-lib/unified-subagents/agent-manager.js");
  ...
  it("top-level spawns default to background, nested spawns to foreground", async () => {
    const { resolveAgentInvocationConfig } = await import("../../pi-toolkit-lib/unified-subagents/invocation-config.js");
  ...
  it("model scope is off by default", async () => {
    ...
    const { isScopeModelsEnabled } = await import("../../pi-toolkit-lib/unified-subagents/model-scope.js");
