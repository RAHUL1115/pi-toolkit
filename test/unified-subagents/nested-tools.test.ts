...
import { getAvailableTypes, registerAgents, setFallbackSubagent } from "../../pi-toolkit-lib/unified-subagents/agent-types.js";
import { loadCustomAgents } from "../../pi-toolkit-lib/unified-subagents/custom-agents.js";
import { setScopeModelsEnabled } from "../../pi-toolkit-lib/unified-subagents/model-scope.js";
import { createNestedSubagentTools, getMaxSubagentDepth, type NestedAgentManager, setMaxSubagentDepth } from "../../pi-toolkit-lib/unified-subagents/nested-tools.js";
import { encodeCwd } from "../../pi-toolkit-lib/unified-subagents/output-file.js";
...
describe("child-safe nested Agent tools", () => {
  ...
  it("rejects native-harness children instead of silently running them through Pi", async () => {
    writeAgent("codex-child", "harness: codex\n");
    const [agent] = tools();
    const result = await execute(agent, {
      subagent_type: "codex-child",
      description: "native child",
      prompt: "Do native work",
    });

    expect(result.isError).toBe(true);
    expect(result.content[0].text).toContain("supports only the Pi harness");
