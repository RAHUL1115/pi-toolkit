...
const PERF_DIR = join("test", "unified-subagents", "perf");
const HELPERS_DIR = join("test", "unified-subagents", "helpers");
const FIXTURES = join(HELPERS_DIR, "perf-fixtures.ts");
...
function runBench(cwd, filter, jsonPath) {
  const args = [join(cwd, "node_modules", "vitest", "vitest.mjs"), "bench", "--run", ...(filter ? [filter] : []), `--outputJson=${jsonPath}`];
  try {
    execFileSync(process.execPath, args, { cwd, stdio: "ignore", timeout: 300_000 });
  } catch {
  ...
  for (const file of report.files ?? []) {
    for (const group of file.groups ?? []) {
      ...
      for (const bench of group.benchmarks ?? []) {
        ...
        const value = bench.median ?? bench.mean;
        if (typeof value === "number" && Number.isFinite(value) && value > 0) {
          out.set(`${groupName} > ${bench.name}`, value);
        }
      }
    }
  }
...
try {
  ...
  mkdirSync(join(worktree, HELPERS_DIR), { recursive: true });
  cpSync(join(REPO, FIXTURES), join(worktree, FIXTURES));
  cpSync(join(REPO, HELPERS_DIR, "boot-extension.ts"), join(worktree, HELPERS_DIR, "boot-extension.ts"));
  ...
  if (head.size === 0 || base.size === 0) {
    throw new Error("No benchmark results on one side; inspect each tree with npm run bench before comparing.");
  }
