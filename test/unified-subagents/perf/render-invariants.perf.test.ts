...
const { AgentWidget } = await import("../../../pi-toolkit-lib/unified-subagents/ui/agent-widget.js");
...
describe("ConversationViewer — cost stays linear in transcript length", () => {
  /** Cold render cost; Toolkit's unchanged-frame cache intentionally does zero leaf work. */
  ...
  // Cold work may scale with transcript length, but not quadratically. Measuring
  // a warm frame here would divide by zero because Toolkit caches stable history.
  it("bounds cold work for 10x the messages (raw wrap path)", () => {
  ...
  it("bounds cold work for 10x the messages (markdown path)", () => {
  ...
  it("does no transcript leaf work for an unchanged warm frame", () => {
    const viewer = mountViewer(ConversationViewer, makeSession(300), undefined, () => "assistant");
    viewer.render(120);
    counts.wrap = 0;
    counts.markdownRender = 0;
    viewer.render(120);
