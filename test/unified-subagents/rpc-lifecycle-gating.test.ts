...
describe("issue #142: RPC handlers + subagents:ready are gated on session_start", () => {
  ...
  beforeEach(() => {
    ...
    writeFileSync(
      ...
      JSON.stringify({ schedulingEnabled: false, fleetView: true }),
    );
  ...
  it("renders an RPC-spawned agent in FleetView while it is running", async () => {
    const { pi, lifecycle, busHandlers } = makePi();
    ...
    try {
      ...
      await vi.waitFor(() => {
        ...
        expect(activeCtx.ui.setWidget.mock.calls.some(
          (call: any[]) => call[0] === "agents" && typeof call[1] === "function",
        )).toBe(false);
      });
    } finally {
  ...
  it("shows live tool activity for an RPC-spawned background agent in FleetView", async () => {
    const { pi, lifecycle, busHandlers } = makePi();
    let widgetFactory: any;
    const setWidget = vi.fn((key: string, content: any) => {
      if (key === "fleet" && content) widgetFactory = content;
    });
