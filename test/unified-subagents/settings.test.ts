...
import {
  ...
  setLightModel,
  setLightThinking,
} from "../../pi-toolkit-lib/unified-subagents/settings.js";
...
describe("settings persistence", () => {
  ...
  it("loads the light configuration only from global settings", () => {
    writeGlobal({ lightModel: "global/light", lightThinking: "high", maxConcurrent: 8 });
    writeProject({ lightModel: "project/light", lightThinking: "max", maxConcurrent: 2 });

    expect(loadSettings(projectDir)).toEqual({
      lightModel: "global/light",
      lightThinking: "high",
      maxConcurrent: 2,
    });
  });

  it("sanitizes the global light model and ignores legacy values", () => {
    writeGlobal({
      lightModel: "  provider/light  ",
      economyModel: "legacy/economy",
      fastModel: "legacy/fast",
    });
    expect(loadSettings(projectDir)).toEqual({ lightModel: "provider/light" });

    writeGlobal({ lightModel: "provider/light", lightThinking: "invalid" });
    expect(loadSettings(projectDir)).toEqual({ lightModel: "provider/light" });
  });

  describe("automatic light model selection", () => {
    const model = (provider: string, id: string) => ({ provider, id });

    it("initializes OpenAI Codex with Sol", () => {
      expect(reconcileLightModel("openai-codex", [
        model("openai-codex", "gpt-5.6-luna"),
        model("openai-codex", "gpt-5.6-sol"),
      ], undefined, false)).toEqual({ lightModel: "openai-codex/gpt-5.6-sol" });
    });

    it("initializes Anthropic with the latest Sonnet", () => {
      expect(reconcileLightModel("anthropic", [
        model("anthropic", "claude-sonnet-4-5"),
        model("anthropic", "claude-haiku-4-6"),
        model("anthropic", "claude-sonnet-4-6"),
      ], undefined, false)).toEqual({ lightModel: "anthropic/claude-sonnet-4-6" });
    });

    it("initializes other providers from their latest catalog entry", () => {
      expect(reconcileLightModel("other", [
        model("other", "v1"),
        model("other", "v2"),
      ], undefined, false)).toEqual({ lightModel: "other/v2" });
    });

    it("preserves an initialized model available under the current provider", () => {
      expect(reconcileLightModel("other", [
        model("other", "custom-light"),
        model("other", "latest"),
      ], "other/custom-light", true)).toBeUndefined();
    });

    it("selects the current provider model after a provider change", () => {
      expect(reconcileLightModel("anthropic", [
        model("openai-codex", "gpt-5.6-sol"),
        model("anthropic", "claude-haiku-4-6"),
        model("anthropic", "claude-sonnet-4-6"),
      ], "openai-codex/gpt-5.6-sol", true)).toEqual({
        lightModel: "anthropic/claude-sonnet-4-6",
      });
    });

    it("falls back to the provider latest model when its preferred model is unavailable", () => {
      expect(reconcileLightModel("openai-codex", [
        model("openai-codex", "gpt-5.6-luna"),
      ], undefined, false)).toEqual({ lightModel: "openai-codex/gpt-5.6-luna" });
    });
  });
  ...
  it("saveSettings never writes the global-only light configuration to the project file", () => {
    saveSettings({ maxConcurrent: 3, lightModel: "provider/light", lightThinking: "high" }, projectDir);
    expect(JSON.parse(readFileSync(projectFile(), "utf-8"))).toEqual({ maxConcurrent: 3 });
  });
  ...
  describe("global light configuration save", () => {
    const light = "provider/light";

    it("creates the global file and writes the model and thinking level", () => {
      expect(saveGlobalLightSettings(light, "high")).toBe(true);
    ...
    it("preserves unrelated fields and removes legacy dual-model fields", () => {
      writeGlobal({
        maxConcurrent: 16,
        futureField: { enabled: true },
        economyModel: "old/economy",
        economyThinking: "max",
        fastModel: "old/fast",
        fastThinking: "low",
      });
      expect(saveGlobalLightSettings(light, "xhigh")).toBe(true);
      expect(JSON.parse(readFileSync(globalFile(), "utf-8"))).toEqual({
        maxConcurrent: 16,
        futureField: { enabled: true },
        lightModel: light,
    ...
    it("does not overwrite malformed or non-object global JSON", () => {
      for (const content of ["not json", "null", "[]", '"text"']) {
        writeFileSync(globalFile(), content);
        expect(saveGlobalLightSettings(light, "low")).toBe(false);
        expect(readFileSync(globalFile(), "utf-8")).toBe(content);
      }
    });
  });
  ...
  describe("applySettings", () => {
    ...
    it("resets the light configuration and leaves other settings unchanged on empty input", () => {
      setLightModel("custom/light");
      setLightThinking("max");
      ...
      expect(getLightThinking()).toBe(DEFAULT_LIGHT_THINKING);
    ...
    it("applies all fields when all are present", () => {
      ...
      expect(getLightModel()).toBe("provider/light");
      expect(getLightThinking()).toBe("xhigh");
    ...
    it("applies the light configuration when present", () => {
      applySettings({ lightModel: "provider/light", lightThinking: "high" }, appliers);
      expect(getLightModel()).toBe("provider/light");
      expect(getLightThinking()).toBe("high");
    });
  ...
  describe("applyAndEmitLoaded", () => {
    ...
    it("still emits the event when both files are missing (payload carries {})", () => {
      ...
      // Operational setters stay untouched; the light configuration resets to its defaults.
      expect(getLightModel()).toBe(DEFAULT_LIGHT_MODEL);
      expect(getLightThinking()).toBe(DEFAULT_LIGHT_THINKING);
      expect(appliers.setMaxConcurrent).not.toHaveBeenCalled();
      expect(appliers.setDefaultMaxTurns).not.toHaveBeenCalled();
