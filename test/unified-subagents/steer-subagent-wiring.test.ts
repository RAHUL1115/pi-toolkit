...
import { runAgent, steerAgent } from "../../pi-toolkit-lib/unified-subagents/agent-runner.js";
...
describe("steer_subagent before the session exists", () => {
  it("queues the message on the record and says so", async () => {
    ...
    expect(pi.events.emit).toHaveBeenCalledWith("subagents:steered", expect.objectContaining({ id, message: "change course" }));
...
describe("steer_subagent once the session exists", () => {
  ...
  it("delivers through steerAgent and announces the steer on success", async () => {
    ...
    expect(pi.events.emit).toHaveBeenCalledWith("subagents:steered", expect.objectContaining({ id, message: "refocus" }));
