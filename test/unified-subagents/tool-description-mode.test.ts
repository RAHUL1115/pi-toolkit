...
import { setWorktreeIsolationEnabled } from "../../pi-toolkit-lib/unified-subagents/worktree.js";

const EXAMPLE_TEMPLATE = fileURLToPath(new URL("../../docs/unified-subagents/examples/agent-tool-description.md", import.meta.url));
...
describe("toolDescriptionMode", () => {
  ...
  it("defaults to the full description", () => {
    ...
    // Full agent descriptions are embedded; light models are variables, not agent types.
    expect(desc).toContain("very thorough");
    expect(desc).not.toContain("- Economy:");
    expect(desc).not.toContain("- Fast:");
    const guidelines = tools.get("Agent").promptGuidelines.join("\n");
    expect(guidelines).toContain('Use thinking: "low"');
    expect(guidelines).toContain('For Pi use model: "openai-codex/gpt-5.6-sol"');
    expect(guidelines).toContain('for Claude use model: "sonnet"');
  });
  ...
  it("light-model routing guidance uses the selected global thinking level", () => {
    const tools = setup({ toolDescriptionMode: "compact" }, () => {
    ...
    const guidelines = tools.get("Agent").promptGuidelines.join("\n");
    expect(guidelines).toContain("fast, low-cost model for straightforward, low-intelligence tasks");
    expect(guidelines).toContain("Choose the agent type by task nature");
    expect(guidelines).toContain("use Explore for read-only search and general-purpose for implementation");
    expect(guidelines).toContain('Use thinking: "xhigh"');
    expect(guidelines).toContain('For Pi use model: "provider/light"');
    expect(guidelines).toContain('for Claude use model: "sonnet"');
    expect(guidelines).toContain('for Codex use model: "gpt-5.6-sol"');
    expect(guidelines).toContain("difficult, open-ended, or high-risk");
  });
  ...
  it("custom mode renders the project template with placeholders substituted", () => {
    ...
    expect(desc).not.toContain("- Economy:");
    expect(desc).not.toContain("- Fast:");
    expect(tools.get("Agent").promptGuidelines.join("\n")).toContain("fast, low-cost model");
