...
vi.mock("../../pi-toolkit-lib/unified-subagents/worktree.js", () => ({
  createWorktree: vi.fn(() => ({ path: "/tmp/wt", workPath: "/tmp/wt", branch: "wf-test", baseSha: "abc" })),
...
import { createWorktree, isWorktreeIsolationEnabled } from "../../pi-toolkit-lib/unified-subagents/worktree.js";
...
describe("the workflow host reports a child's effective configuration", () => {
  ...
  it("preserves local frontmatter precedence over a script model hint", async () => {
    const haiku = { provider: "anthropic", id: "claude-haiku-4-5", name: "Haiku 4.5" };
  ...
  it("lets frontmatter isolation veto a workflow worktree request", async () => {
    vi.mocked(isWorktreeIsolationEnabled).mockReturnValue(true);
    registerAgents(new Map([["shared", { name: "shared", isolation: "off" } as any]]));
    childSessionReports({ model: { provider: "anthropic", id: "claude-sonnet-4-6" } });
    const host = createWorkflowHost({ pi, ctx: ctx({}), manager });

    await host.spawnAgent(spawnRequest({ agentType: "shared", isolation: "worktree" }));

    expect(createWorktree).not.toHaveBeenCalled();
    expect(vi.mocked(runAgent).mock.calls[0]?.[3].worktreeBase).toBeUndefined();
  });

  it("drops frontmatter worktree isolation when the project disables it", async () => {
    registerAgents(new Map([["isolated", { name: "isolated", isolation: "worktree" } as any]]));
    childSessionReports({ model: { provider: "anthropic", id: "claude-sonnet-4-6" } });
    const host = createWorkflowHost({ pi, ctx: ctx({}), manager });

    await host.spawnAgent(spawnRequest({ agentType: "isolated" }));
