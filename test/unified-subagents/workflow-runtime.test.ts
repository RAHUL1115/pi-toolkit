...
import type { WorkflowJournalEntry } from "../../pi-toolkit-lib/unified-subagents/workflow/journal.js";
import { buildPhaseGroups, type WorkflowAgentEntry, type WorkflowEntry } from "../../pi-toolkit-lib/unified-subagents/workflow/progress.js";
import {
  assertBoundarySafe,
  ...
  workflowConcurrency,
} from "../../pi-toolkit-lib/unified-subagents/workflow/runtime.js";
...
describe("the worker source itself", () => {
  it("parses as JavaScript", async () => {
    ...
    const { WORKER_SOURCE } = await import("../../pi-toolkit-lib/unified-subagents/workflow/worker-source.js");
...
describe("script globals", () => {
  ...
  it("validates and carries opts.harness to the host", async () => {
    const { host, calls } = stubHost();
    const result = await run('await agent("native", { harness: "claude" });\nreturn null;', { host });
  ...
  it("rejects an unknown harness before spawning", async () => {
    const { host, calls } = stubHost();
    const result = await run('await agent("bad", { harness: "other" });\nreturn null;', { host });

    expect(result.status).toBe("failed");
    expect(result.error).toContain("opts.harness must be one of");
