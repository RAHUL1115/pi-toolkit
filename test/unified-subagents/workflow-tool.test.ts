...
import { SUBAGENT_TOOL_NAMES } from "../../pi-toolkit-lib/unified-subagents/agent-runner.js";
import { NO_FALLBACK, registerAgents, setFallbackSubagent } from "../../pi-toolkit-lib/unified-subagents/agent-types.js";
import { registerUnifiedSubagents as subagentsExtension, WORKFLOW_ENTRY_TYPE, WORKFLOW_FILE_FLAG } from "../../pi-toolkit-lib/unified-subagents/index.js";
import { isScopeModelsEnabled, setScopeModelsEnabled } from "../../pi-toolkit-lib/unified-subagents/model-scope.js";
...
function stubManager(
  ...
  const resume = vi.fn(async (id: string) => {
    const resumed = record({ id, result: "resumed" });
    records.set(id, resumed);
    return resumed;
  });
  const getRecord = vi.fn((id: string) => records.get(id));
  ...
  return {
    manager: { spawnAndWait, abort, resume, getRecord } as unknown as AgentManager,
    spawnAndWait,
    abort,
    resume,
    getRecord,
...
describe("SubagentWorkflow tool — script vs scriptPath vs name", () => {
  ...
  it("settles a workflow worker on /new without leaking its resume handle or notification", async () => {
    // A never-returning script: only the run's own abort signal can stop it, so
    // abortAll() over agent records is not enough. Pi fires before_switch first,
    // then shutdown only after the switch is accepted.
    const result = await tools.get("SubagentWorkflow").execute(
      "tc-shutdown",
    ...
    await booted.lifecycle.get("session_before_switch")?.({ reason: "new" }, workflowCtx());
    await booted.lifecycle.get("session_shutdown")?.({ reason: "new" }, workflowCtx());
    await new Promise(resolve => setTimeout(resolve, 250));

    // The old registrar is fenced before aborting: its detached continuation
    // must not send into the replacement session after shutdown.
    expect(
      booted.pi.sendMessage.mock.calls.some((c: any[]) => String(c[0]?.content).includes(taskId)),
    ).toBe(false);

    // The per-session map is gone too, so its journal cannot be resumed through
    // a stale run id even if an old tool reference is invoked by the harness.
    const resumed = await tools.get("SubagentWorkflow").execute(
      "tc-after-new",
      { script: inlineScript, resumeFromRunId: taskId },
      undefined, undefined, workflowCtx(),
    );
    expect(textOf(resumed)).toMatch(new RegExp(`No workflow run "${taskId}" in this session`));

    // The registrar itself survives replacement, so the replacement session
    // must accept new runs even though callbacks from the old map stay fenced.
    await booted.lifecycle.get("session_start")?.({}, workflowCtx());
    const replacement = await tools.get("SubagentWorkflow").execute(
      "tc-after-new-run",
      { script: inlineScript },
      undefined, undefined, workflowCtx(),
    );
    const replacementId = startedTaskId(replacement);
    const sent = await awaitNotification(replacementId);
    expect(sent[0].details.status).toBe("completed");
    expect(String(sent[0].content)).toContain("<status>Done</status>");
  });
