...
} from "../../pi-toolkit-lib/unified-subagents/worktree.js";
...
describe("cleanupWorktree — failure path", () => {
  ...
  it("swallows a git failure inside a still-present worktree and reports no changes", async () => {
    ...
    // Break the worktree's link back to the repo. Git marks this file hidden on Windows.
    const gitFile = join(wt.path, ".git");
    if (process.platform === "win32") execFileSync("attrib", ["-H", gitFile]);
    writeFileSync(gitFile, "gitdir: /nonexistent/path/that/is/not/a/repo");
